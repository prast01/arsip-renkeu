					...
					<tr class="gradeX">
						<td>Jenis Surat
							<select name="cjnssurat" id="cjnssurat" class="">
								  ...
								  <?php   
								  $query=mysql_query("select * from jns_surat where ksurat");
								  ...
								  if ($cjnssurat!=""){
								  $query=mysql_query("select * from jns_surat where ksurat='$cjnssurat'");
								  while($row=mysql_fetch_array($query)){
								  $ksurat = $row['ksurat'];
								  ?>
								  <option value="<?php  echo $row['ksurat']; ?>">
							...
							<a href="cetak_laporan_harian.php?cjnssurat=<?php echo $cjnssurat ?>&cbulan=<?php echo $cbulan ?>&ctahun=<?php echo $ctahun ?>" target="_blank"><input type="button" value="Cetak">
							</a>
...
<form method="post" enctype="multipart/form-data">
  ...
  $cjnssurat = isset($_POST['cjnssurat']) ? $_POST['cjnssurat'] : '';
	...
	if($lihat){
		$sql = mysql_query("SELECT * from jns_surat where ksurat like '%$cjnssurat%' order by ksurat")or die(mysql_error());
		} else {
  ...
  <?php 
                        ...
                        <tr>
                            <?php
                            if($cbulan=="02"){
                                $jtp = mysql_query("SELECT count(id) as jmlkasus FROM transaksi WHERE tanggal>='$ctahun-$cbulan-01' AND tanggal<='$ctahun-$cbulan-29' AND ksurat like '%$cjnssurat%'")or die(mysql_error());
                            }elseif ($cbulan=="01" || $cbulan=="03" || $cbulan=="05" || $cbulan=="07" || $cbulan=="08" || $cbulan=="10" || $cbulan=="12") {
                                $jtp = mysql_query("SELECT count(id) as jmlkasus FROM transaksi WHERE tanggal>='$ctahun-$cbulan-01' AND tanggal<='$ctahun-$cbulan-31' AND ksurat like '%$cjnssurat%'")or die(mysql_error());    
                            }elseif($cbulan=="04" || $cbulan=="06" || $cbulan=="09" || $cbulan=="11") {
                                $jtp = mysql_query("SELECT count(id) as jmlkasus FROM transaksi WHERE tanggal>='$ctahun-$cbulan-01' AND tanggal<='$ctahun-$cbulan-30' AND ksurat like '%$cjnssurat%'")or die(mysql_error());
                            }else{
